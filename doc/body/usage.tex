...
\begin{function}[updated = 2025-11-07]{\fillin}
  ...
  \begin{itemize}
    \item \textbf{显示答案时}（\kvopt{show-answer}{true}）：
      \begin{itemize}
        \item \tn{fillin}：答案内容\textbf{不可换行}，但会自动根据内容深度提升基线（如排版分数不会"压线"）
        \item \tn{fillin*}：答案内容同样\textbf{不可换行}
      \end{itemize}
    ...
    \item \textbf{不显示答案时}（\kvopt{show-answer}{false}）：
      \begin{itemize}
        ...
        \item \tn{fillin*}：\textbf{可以自动换行}，但没有基线提升功能。换行功能仅适用于 \kvopt{fillin/type}{line}、\kvopt{fillin/type}{paren} 和 \kvopt{fillin/type}{blank}
      \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    \item 短答案（如数字、单词）：使用 \tn{fillin}
    \item 长答案需要换行：使用 \tn{fillin*}，并设置 \kvopt{show-answer}{false}
...
\begin{latexexample}{\tn{fillin} 和 \tn{fillin*} 的换行行为对比}
  ...
  长答案（不换行）：\fillin[$\frac{1}{2}$ 这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容。]

  长答案（换行）：\fillin*[$\frac{1}{2}$ 这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容。]
