...
\begin{function}{\varnothing}
  空集符号。为使不同数学字体下的字形保持一致，\cls{exam-zh} 会在文档开始时
  单独使用 Asana Math 的字形覆盖 |\varnothing|；该处理不改变其余数学符号，
  也不表示模板加载或完整支持商业宏包 \pkg{mtpro2}。如需完全沿用所选数学字体，
  可在 |\begin{document}| 之后再次用 \pkg{unicode-math} 的 |\setmathfont|
  按范围覆盖该符号。
\end{function}
...
\begin{function}[added = 2022-07-03]{\information}
  \begin{examzhsyntax}[emph={[1]information}]
...
\begin{function}[added = 2022-07-03]{\warning}
  \begin{examzhsyntax}[emph={[1]warning}]
    \warning(*\marg{警告}*)
  \end{examzhsyntax}
  警告命令。居中、黑体。使用示例：
  \begin{latexcode}[gobble=4]
...
\begin{function}[updated = 2022-07-03]{\secret}
  \begin{examzhsyntax}[emph={[1]secret}]
    \secret(*\oarg{格式命令}*)
  \end{examzhsyntax}
...
%   \begin{examzhsyntax}[emph={[1]goodluck}]
%     \goodluck(*\oarg{祝福语}*)
%   \end{examzhsyntax}
%   祝福语命令。祝福语默认为 |祝考试顺利|。
% \end{function}
...
\begin{function}[updated = 2022-07-26]{notice 环境}
  \begin{examzhsyntax}[emph={[2]notice}]
...
\begin{function}[added = 2022-07-26]{notice/label}
  \begin{examzhsyntax}[emph={[1]label}]
    label = (*\meta{label}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-26]{notice/label-format}
  \begin{examzhsyntax}[emph={[1]label-format}]
...
\begin{function}[added = 2022-07-26]{notice/top-sep,notice/bottom-sep}
  \begin{examzhsyntax}[emph={[1]top-sep,bottom-sep}]
    top-sep = (*\meta{skip}*)
...
\begin{function}{\title}
  \begin{examzhsyntax}[emph={[1]title}]
    \title(*\marg{标题}*)
  \end{examzhsyntax}
...
\begin{function}[updated = 2022-07-24]{\subject}
  \begin{examzhsyntax}[emph={[1]subject}]
...
\begin{function}{\maketitle}
  \begin{examzhsyntax}[emph={[1]maketitle}]
    \maketitle
  \end{examzhsyntax}
  生成标题和科目。
\end{function}
...
\begin{function}{question 环境}
  \begin{examzhsyntax}[emph={[2]question}]
...
\begin{function}{problem 环境}
  \begin{examzhsyntax}[emph={[2]problem}]
...
\begin{function}{\paren}
  \begin{examzhsyntax}[emph={[2]\paren}]
    \paren(*\oarg{答案}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-20]{\AddQuestionCounter}
  \begin{examzhsyntax}[emph={[2]\AddQuestionCounter}]
    \AddQuestionCounter(*\marg{LaTeX command}*)(*\marg{internal command}*)
  \end{examzhsyntax}
  如果用户需要使用其它形式的数字作为 \env{question} 环境和 \env{problem} 的标签，需要使用 \tn{AddQuestionCounter} 命令将其添加进 \opt{label} 选项的识别范围内（类似 \pkg{enumitem} 宏包的 \tn{AddEnumerateCounter} ）。其中 \meta{LaTeX command} 是在 \opt{label} 选项中的形式，\meta{internal command} 是内部的实现。比如带圈数字的添加方法：
  \begin{latexcode}[gobble=4]
    \AddQuestionCounter{\circlednumber}{\__examzh_font_circled_number:n}
  \end{latexcode}
\end{function}
...
\begin{function}{choices 环境}
  \begin{examzhsyntax}[emph={[2]choices}]
...
\begin{function}{\setchoices}
  \begin{examzhsyntax}[emph={[2]\setchoices}]
...
\begin{function}{\AddChoicesCounter}
  \begin{examzhsyntax}[emph={[2]\AddChoicesCounter}]
    \AddChoicesCounter(*\marg{LaTeX command}*)(*\marg{internal command}*)
  \end{examzhsyntax}
  如果用户需要使用其它形式的数字作为 \env{choices} 环境的标签，需要使用 \tn{AddChoicesCounter} 命令将其添加进 \opt{label} 选项的识别范围内（类似 \pkg{enumitem} 宏包的 \tn{AddEnumerateCounter} ）。其中 \meta{LaTeX command} 是在 \opt{label} 选项中的形式，\meta{internal command} 是内部的实现。比如带圈数字的添加方法：
  \begin{latexcode}[gobble=4]
    \AddChoicesCounter{\circlednumber}{\__examzh_font_circled_number:n}
  \end{latexcode}
\end{function}
...
\begin{function}[updated = 2026-07-11]{\circlednumber,\tikzcirclednumber}
  \begin{examzhsyntax}[emph={[2]\circlednumber,\tikzcirclednumber}]
    \circlednumber(*\meta{数字或计数器名字}*)
    \tikzcirclednumber(*\meta{数字或计数器名字}*)
  \end{examzhsyntax}
  带圈数字命令。\tn{circlednumber} 基于字体开发，仅接受 0～50 的输入值；\tn{tikzcirclednumber} 基于 \TikZ 开发，无此限制。两者均接受整数表达式或 LaTeX 计数器名。在 \pkg{enumitem} 的 \env{enumerate} 以及本模板的 \env{question}、\env{problem}、\env{choices} 和填空编号中，统一使用末尾星号作为当前项编号占位符，例如 |label=\circlednumber*| 或 |label=\tikzcirclednumber*|。

  旧写法 |\circlednumber*{数字或计数器名字}| 暂时保留兼容，但已弃用；请改用 \tn{tikzcirclednumber}，以免与标签中的星号占位符混淆。
  \begin{latexexample}{\tn{circlednumber} 的使用示例}
    \circlednumber{1} \circlednumber{2}
    \tikzcirclednumber{1} \tikzcirclednumber{2}
...
\begin{function}[updated = 2026-07-12]{\fillin}
  \begin{examzhsyntax}[emph={[2]\fillin}]
    \fillin(*\oarg{键值列表}*)(*\oarg{答案}*)
    \fillin*(*\oarg{键值列表}*)(*\oarg{答案}*)
  \end{examzhsyntax}
  填空命令，可生成下划线或括号等形式。\meta{答案} 可以受~\ref{subsubsec:参数-题干} 节的 \cmd{question/show-answer} 键值控制显示或隐藏。\meta{键值列表} 见~\ref{subsubsec:参数-填空题} 节。

  \paragraph{换行行为说明}

  \tn{fillin} 和 \tn{fillin*} 的换行行为取决于是否显示答案：

  \begin{itemize}
    \item \textbf{显示答案时}（\kvopt{show-answer}{true}）：
      \begin{itemize}
        \item \tn{fillin}：短答案保持单行并自动提升高公式；当答案自然宽度超过当前 \tn{linewidth} 时，\opt{line}、\opt{paren} 和 \opt{blank} 类型自动切换到可换行排版
        \item \tn{fillin*}：无论答案长短都强制使用可换行排版，保留为兼容和显式控制接口
        \item 两种写法进入多行排版后，都会根据答案最大深度自动下移装饰线，避免分式等高公式压线；公式内部若没有可断点，仍需用户手动拆分
      \end{itemize}

    \item \textbf{不显示答案时}（\kvopt{show-answer}{false}）：
      \begin{itemize}
        \item \tn{fillin}：生成固定长度的占位符（下划线/括号等），默认\textbf{不换行}。\emph{v0.2.6 起}，当 \kvopt{no-answer-type}{none} 且宽度超出行宽时，会自动换行
        \item \tn{fillin*}：强制使用可换行占位符。换行功能仅适用于 \kvopt{fillin/type}{line}、\kvopt{fillin/type}{paren} 和 \kvopt{fillin/type}{blank}
      \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    \item 一般情况统一使用 \tn{fillin}；显示答案时会根据内容宽度自动选择单行或多行排版
    \item 需要明确强制可换行时使用 \tn{fillin*}
    \item 宽度超出行宽的占位符：使用 \tn{fillin} 配合 \kvopt{no-answer-type}{none}（v0.2.6+）
  \end{itemize}
\end{function}
...
需要注意的是，如果 \tn{fillin} 的参数中含有不配对的中括号时会报错，如 |\fillin[$(−\infty, 1]$]|。这时需要使用大括号将内容保护起来：|\fillin[{$(−\infty, 1]$}]|。

\begin{latexexample}{\tn{fillin} 和 \tn{fillin*} 的换行行为对比}
  ...
  长答案（自动换行）：\fillin[$\frac{1}{2}$ 这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容，这是一个很长的答案内容。]
  ...
  默认固定长度：\fillin[width=5em][答案]

  超宽自动换行（v0.2.6+）：
  测试 \fillin[width=1.2\linewidth][长答案]

  使用 \verb|\fillin*| 换行：
  测试 \fillin*[width=1.2\linewidth][长答案内容]
\end{latexexample}


\begin{function}[added = 2022-07-21]{\AddFillinCounter}
  \begin{examzhsyntax}[emph={[1]\AddFillinCounter}]
    \AddFillinCounter(*\marg{LaTeX command}*)(*\marg{internal command}*)
  \end{examzhsyntax}
  如果用户需要使用其它形式的数字作为 \kvopt{fillin/no-answer-type}{counter} 下 \opt{counter} 的标签，需要使用 \tn{AddFillinCounter} 命令将其添加进 \opt{label} 选项的识别范围内（类似 \pkg{enumitem} 宏包的 \tn{AddEnumerateCounter} ）。其中 \meta{LaTeX command} 是在 \opt{label} 选项中的形式，\meta{internal command} 是内部的实现。比如带圈数字的添加方法：
  \begin{latexcode}[gobble=4]
    \AddFillinCounter{\circlednumber}{\__examzh_font_circled_number:n}
  \end{latexcode}
\end{function}
...
\begin{function}[added = 2022-07-01, updated = 2022-07-19]{solution 环境}
  \begin{examzhsyntax}[emph={[2]solution}]
...
\begin{function}[added = 2022-07-01]{\score}
  \begin{examzhsyntax}[emph={[1]score}]
    \score(*\marg{分数}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-04]{step 环境}
  \begin{examzhsyntax}[emph={[2]step}]
    \begin{step}
      \item ...
      \item ...
    \end{step}
...
\begin{function}[added = 2022-07-04]{method 环境}
  \begin{examzhsyntax}[emph={[2]method}]
    \begin{method}
      \item ...
      \item ...
    \end{method}
...
\begin{function}[added = 2022-07-04]{case 环境}
  \begin{examzhsyntax}[emph={[2]case}]
    \begin{case}
      \item ...
      \item ...
    \end{case}
...
\begin{function}[added = 2022-07-03]{\draftpaper}
  \begin{examzhsyntax}[emph={[1]draftpaper}]
...
\begin{function}[added = 2022-07-04]{\examsquare}
  \begin{examzhsyntax}[emph={[1]examsquare}]
...
\begin{function}[added = 2022-07-04]{\scoringbox,\scoringbox*}
  \begin{examzhsyntax}[emph={[1]scoringbox}]
    \scoringbox
    \scoringbox*
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-19,updated = 2022-07-23]{select 环境}
  \begin{examzhsyntax}[emph={[2]select}]
...
\begin{function}[added = 2022-07-19]{lineto 环境,\linelistset,\lineconnect}
  \begin{examzhsyntax}[emph={[2]lineto,linelistset,lineconnect}]
...
\begin{function}[added = 2022-07-24]{material 环境}
  \begin{examzhsyntax}[emph={[2]material}]
...
\begin{function}[added = 2022-07-24,updated = 2022-07-26]{poem 环境}
  \begin{examzhsyntax}[emph={[2]poem}]
...
\begin{function}[added = 2022-07-26]{writingbox 环境}
  \begin{examzhsyntax}[emph={[2]writingbox}]
...
\begin{function}[added = 2022-07-21]{\ExamPrintAnswerSet}
  \begin{examzhsyntax}[emph={[1]ExamPrintAnswerSet}]
...
如果教师版需要使用独立卷头，并隐藏仅面向考生的警告和注意事项，可以把
这些覆盖命令放入第一个可选参数。它们只会在第二个版本进入
\env{document} 前执行，不会改变正常编译得到的学生版：
...
其中重新定义 \env{notice} 时需要使用 |+b| 捕获并丢弃环境正文；否则正文中的
\tn{item} 仍会继续执行。未在 \tn{ExamPrintAnswerSet} 中覆盖的标题、科目和
其他设置会继承学生版当前值。


\begin{function}[added = 2022-07-21]{\ExamPrintAnswer}
  \begin{examzhsyntax}[emph={[1]ExamPrintAnswer}]
    \ExamPrintAnswer
  \end{examzhsyntax}
...
\begin{function}[added = 2022-08-28]{multifigures 环境}
  \begin{examzhsyntax}[emph={[2]multifigures}]
...
\begin{function}[added = 2022-08-28]{\textfigure}
  \begin{examzhsyntax}[emph={[2]textfigure}]
...
\begin{function}[added = 2024-02-11]{calculations 环境}
  \begin{examzhsyntax}[emph={[2]calculations}]
...
\begin{function}{\examsetup}
  \begin{examzhsyntax}[morekeywords={\examsetup}]
    \examsetup(*\marg{键值列表}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-20]{style}
  \begin{examzhsyntax}[emph={[1]style}]
    style = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-20]{style/fullwidth-stop}
  \begin{examzhsyntax}[emph={[1]fullwidth-stop}]
...
\begin{function}[added = 2022-07-24]{style/footnote-style}
  % 这里奇怪的东西是用来控制对齐的。examzhsyntax 会吃掉开头的几个空格，因此这里用 X 来占位。
  \begin{examzhsyntax}[emph={[1]footnote-style}]
...
\begin{function}[added = 2022-07-22]{style/student-version-suffix}
  \begin{examzhsyntax}[emph={[1]student-version-suffix}]
...
\begin{function}[added = 2022-07-22]{style/student-version-cleanaux}
  \begin{examzhsyntax}[emph={[1]student-version-cleanaux}]
...
\begin{function}{symbols}
  \begin{examzhsyntax}[emph={[1]symbols}]
    symbols = (*\marg{键值列表}*)
...
\begin{function}{symbols/paralleleq-type}
  \begin{examzhsyntax}[emph={[1]paralleleq-type}]
...
\begin{function}[added = 2022-07-17]{symbols/change-frac-style,symbols/change-dfrac-style}
  \begin{examzhsyntax}[emph={[1]change-frac-style,change-dfrac-style}]
...
\begin{function}[added = 2022-07-17]{symbols/frac-add,symbols/dfrac-add}
  \begin{examzhsyntax}[emph={[1]frac-add,dfrac-add}]
...
\begin{function}{page}
  \begin{examzhsyntax}[emph={[1]page}]
    page = (*\marg{键值列表}*)
...
\begin{function}{page/size}
  \begin{examzhsyntax}[emph={[1]size}]
    size = (*<a3paper|a4paper|(custom)>*)
  \end{examzhsyntax}
  试卷尺寸。\opt{a4paper} 表示一页为 A4 纸大小；\opt{a3paper} 表示一页为 A3 纸大小，内容为连续两页 A4 拼接；\opt{custom} 使用 \opt{page/paper-width} 与 \opt{page/paper-height} 指定任意单页纸张尺寸。此设置只能放在导言区。
\end{function}

\begin{function}[added = 2026-07-11]{page/paper-width,page/paper-height,page/margin,page/headheight}
  \begin{examzhsyntax}[emph={[1]paper-width,paper-height,margin,headheight}]
    paper-width  = (*\meta{dimension}*)
    paper-height = (*\meta{dimension}*)
    margin       = (*\meta{dimension}*)
    headheight   = (*\meta{dimension}*)
  \end{examzhsyntax}
  自定义纸张的宽、高、页边距和页眉高度。仅当 \kvopt{page/size}{custom} 时，\opt{paper-width} 与 \opt{paper-height} 生效且必须为正值；\opt{margin} 和 \opt{headheight} 分别默认为 |1in| 和 |0.7in|。自定义纸张采用普通单页页码和居中页脚模型，不会继承 \opt{a3paper} 的横向双页拼版、双栏和分栏页脚行为。
\end{function}
...
\begin{function}[added = 2022-09-12]{page/show-head}
  \begin{examzhsyntax}[emph={[1]show-head}]
    show-head = (*\TFF*)
  \end{examzhsyntax}
  是否显示页眉。
\end{function}
...
\begin{function}[added = 2022-09-12]{page/show-foot}
  \begin{examzhsyntax}[emph={[1]show-foot}]
    show-foot = (*\TTF*)
  \end{examzhsyntax}
  是否显示页脚。
\end{function}
...
\begin{function}[added = 2022-09-12]{page/head-content}
  \begin{examzhsyntax}[emph={[1]head-content}]
...
\begin{function}[updated = 2022-07-24]{page/foot-type}
  \begin{examzhsyntax}[emph={[1]foot-type}]
...
\begin{function}[added = 2022-07-04]{page/foot-content}
  \begin{examzhsyntax}[emph={[1]foot-content}]
...
\begin{function}[added = 2022-07-04]{page/show-columnline}
  \begin{examzhsyntax}[emph={[1]show-columnline}]
    show-columnline = (*\TTF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-04]{page/columnline-width}
  \begin{examzhsyntax}[emph={[1]columnline-width}]
...
\begin{function}[added = 2022-07-11]{page/show-chapter}
  \begin{examzhsyntax}[emph={[1]show-chapter}]
    show-chapter = (*\TTF*)
  \end{examzhsyntax}
...
\begin{function}{sealline}
  \begin{examzhsyntax}[emph={[1]sealline}]
    sealline = (*\marg{键值列表}*)
...
\begin{function}{sealline/show}
  \begin{examzhsyntax}[emph={[1]show}]
    show = (*\TFF*)
  \end{examzhsyntax}
  是否显示密封线。
\end{function}
...
\begin{function}[updated = 2022-07-26]{sealline/scope,sealline/type}
  \begin{examzhsyntax}[emph={[1]scope,type}]
...
\begin{function}{sealline/line-thickness,sealline/odd-line-thickness,sealline/even-line-thickness}
  \begin{examzhsyntax}[emph={[1]line-thickness,odd-line-thickness,even-line-thickness}]
...
\begin{function}{sealline/line-xshift,sealline/odd-line-xshift,sealline/even-line-xshift}
  \begin{examzhsyntax}[emph={[1]line-xshift,odd-line-xshift,even-line-xshift}]
...
\begin{function}{sealline/line-yshift,sealline/odd-line-yshift,sealline/even-line-yshift}
  \begin{examzhsyntax}[emph={[1]line-yshift,odd-line-yshift,even-line-yshift}]
...
\begin{function}{sealline/line-type,sealline/odd-line-type,sealline/even-line-type}
  \begin{examzhsyntax}[emph={[1]line-type,odd-line-type,even-line-type}]
...
\begin{function}{sealline/text,sealline/odd-text,sealline/even-text}
  \begin{examzhsyntax}[emph={[1]text,odd-text,even-text}]
    text = (*\meta{content}*)
...
\begin{function}[added = 2022-07-24]{sealline/text-width,sealline/odd-text-width,sealline/even-text-width}
  \begin{examzhsyntax}[emph={[1]text-width,odd-text-width,even-text-width}]
...
\begin{function}[added = 2022-07-24]{sealline/text-format,sealline/odd-text-format,sealline/even-text-format}
  \begin{examzhsyntax}[emph={[1]text-format,odd-text-format,even-text-format}]
...
\begin{function}{sealline/text-xshift,sealline/odd-text-xshift,sealline/even-text-xshift}
  \begin{examzhsyntax}[emph={[1]text-xshift,odd-text-xshift,even-text-xshift}]
...
\begin{function}[updated = 2022-07-24]{sealline/text-yshift,sealline/odd-text-yshift,sealline/even-text-yshift}
  \begin{examzhsyntax}[emph={[1]text-yshift,odd-text-yshift,even-text-yshift}]
...
\begin{function}[added = 2022-07-24]{sealline/text-xscale,sealline/odd-text-xscale,sealline/even-text-xscale}
  \begin{examzhsyntax}[emph={[1]text-xscale,odd-text-xscale,even-text-xscale}]
...
\begin{function}[added = 2022-07-24]{sealline/text-yscale,sealline/odd-text-yscale,sealline/even-text-yscale}
  \begin{examzhsyntax}[emph={[1]text-yscale,odd-text-yscale,even-text-yscale}]
...
\begin{function}[added = 2022-07-24]{sealline/text-direction-vertical,sealline/odd-text-direction-vertical,sealline/even-text-direction-vertical}
  \begin{examzhsyntax}[emph={[1]text-direction-vertical,odd-text-direction-vertical,even-text-direction-vertical}]
...
\begin{function}{sealline/circle-show,sealline/odd-circle-show,sealline/even-circle-show}
  \begin{examzhsyntax}[emph={[1]circle-show,odd-circle-show,even-circle-show}]
    circle-show = (*\TTF*)
    odd-circle-show = (*\TTF*)
...
\begin{function}{sealline/circle-start,sealline/odd-circle-start,sealline/even-circle-start,sealline/circle-end,sealline/odd-circle-end,sealline/even-circle-end}
  \begin{examzhsyntax}[emph={[1]circle-start,odd-circle-start,even-circle-start,circle-end,odd-circle-end,even-circle-end}]
...
\begin{function}{sealline/circle-step,sealline/odd-circle-step,sealline/even-circle-step}
  \begin{examzhsyntax}[emph={[1]circle-step,odd-circle-step,even-circle-step}]
...
\begin{function}{sealline/circle-diameter,sealline/odd-circle-diameter,sealline/even-circle-diameter}
  \begin{examzhsyntax}[emph={[1]circle-diameter,odd-circle-diameter,even-circle-diameter}]
...
\begin{function}{sealline/circle-xshift,sealline/odd-circle-xshift,sealline/even-circle-xshift}
  \begin{examzhsyntax}[emph={[1]circle-xshift,odd-circle-xshift,even-circle-xshift}]
...
\begin{function}{sealline/odd-info-content}
  \begin{examzhsyntax}[emph={[1]odd-info-content}]
...
\begin{function}{sealline/odd-info-separator}
  \begin{examzhsyntax}[emph={[1]odd-info-separator}]
...
\begin{function}{sealline/odd-info-align}
  \begin{examzhsyntax}[emph={[1]odd-info-align}]
...
\begin{function}{sealline/odd-info-xshift}
  \begin{examzhsyntax}[emph={[1]odd-info-xshift}]
...
\begin{function}{sealline/odd-info-yshift}
  \begin{examzhsyntax}[emph={[1]odd-info-yshift}]
...
\begin{function}{square}
  \begin{examzhsyntax}[emph={[1]square}]
    square = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-04,updated = 2022-07-23]{square/x-length}
  \begin{examzhsyntax}[emph={[1]x-length}]
...
\begin{function}[added = 2022-07-04,updated = 2022-07-23]{square/baseline}
  \begin{examzhsyntax}[emph={[1]baseline}]
...
\begin{function}[added = 2022-07-04,updated = 2022-07-23]{square/linewidth}
  \begin{examzhsyntax}[emph={[1]linewidth}]
...
\begin{function}[added = 2022-07-04,updated = 2022-07-23]{square/xshift}
  \begin{examzhsyntax}[emph={[1]xshift}]
...
\begin{function}[updated = 2022-08-15]{font}
  \begin{examzhsyntax}[emph={[1]font}]
...
\begin{function}[updated = 2022-08-15]{math-font}
  \begin{examzhsyntax}[emph={[1]math-font}]
...
\begin{function}[added = 2022-07-03]{title/title-format}
  \begin{examzhsyntax}[emph={[1]title-format}]
...
\begin{function}[added = 2022-07-03]{title/subject-format}
  \begin{examzhsyntax}[emph={[1]subject-format}]
...
\begin{function}[added = 2022-07-03]{title/top-sep}
  \begin{examzhsyntax}[emph={[1]top-sep}]
    top-sep = (*\meta{弹性长度}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-03]{title/bottom-sep}
  \begin{examzhsyntax}[emph={[1]bottom-sep}]
...
\begin{function}{question}
  \begin{examzhsyntax}[emph={[1]question}]
    question = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-09-18]{problem}
  \begin{examzhsyntax}[emph={[1]problem}]
    problem = (*\marg{键值列表}*)
...
\begin{function}[updated = 2022-07-05]{question/show-answer}
  \begin{examzhsyntax}[emph={[1]show-answer}]
    show-answer = (*\TFF*)
  \end{examzhsyntax}
...
\begin{function}{question/points}
  \begin{examzhsyntax}[emph={[1]points}]
    points = (*\meta{color}*)
  \end{examzhsyntax}
...
\begin{function}{question/show-points}
  \begin{examzhsyntax}[emph={[1]show-points}]
...
\begin{function}{question/points-separate-par}
  \begin{examzhsyntax}[emph={[1]points-separate-par}]
...
\begin{function}{question/index}
  \begin{examzhsyntax}[emph={[1]index}]
    index = (*\meta{integer}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-26]{question/combine-fillin}
  \begin{examzhsyntax}[emph={[1]combine-fillin}]
    combine-fillin = (*\TFF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-26]{question/combine-fillin-args}
  \begin{examzhsyntax}[emph={[1]combine-fillin-args}]
...
\begin{function}[added = 2022-08-12]{question/hang}
  \begin{examzhsyntax}[emph={[1]hang}]
    hang = (*\TTF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2026-07-11]{problem/hang}
  \begin{examzhsyntax}[emph={[1]hang}]
    hang = (*<true|(false)>*)
  \end{examzhsyntax}
  控制 \env{problem} 题干是否采用悬挂缩进。默认值为 \opt{false}，保持解答题题干首行缩进；设为 \opt{true} 后，题干的续行与题号后的首行对齐。该设置不改变内层 \env{enumerate} 的列表层级和缩进。
\end{function}

\begin{latexexample}{\opt{problem/hang} 的效果}
  \begin{problem}
    默认解答题题干保持首行缩进。题干较长并换行时，后续文字从页面左侧开始。
  \end{problem}

  \begin{problem}[hang = true]
    悬挂模式下，题干换行后的文字与题号后的首行对齐。
  \end{problem}
\end{latexexample}
...
\begin{function}[added = 2022-09-18]{question/points-prelabel,question/points-postlabel}
  \begin{examzhsyntax}[emph={[1]points-prelabel,points-postlabel}]
...
\begin{function}{choices}
  \begin{examzhsyntax}[emph={[1]choices}]
    choices = (*\marg{键值列表}*)
...
\begin{function}[added = 2026-07-12]{choices/show-answer,choices/answer-color}
  \begin{examzhsyntax}[emph={[1]show-answer,answer-color}]
    show-answer = (*\TFF*)
    answer-color = (*\meta{color}*)
  \end{examzhsyntax}
  在 \env{choices} 中使用 \tn{item} 后紧跟星号（|\item*|）标记正确选项。默认的 \kvopt{show-answer}{false} 会移除源码中的星号但不改变选项外观；设置为 \opt{true} 后，正确选项的标签和内容使用 \opt{answer-color} 着色，默认颜色为 \opt{red}。该接口只负责当前选项列表的答案显隐，不会收集或移动答案，也不与 \opt{question/show-answer} 自动联动。
\end{function}

\begin{latexexample}{使用 \tn{item*} 标记并显示正确选项}
  \begin{choices}[show-answer = true, answer-color = blue]
    \item 错误选项
    \item* 正确选项
    \item 错误选项
    \item* 另一个正确选项
...
\begin{function}{choices/column-sep}
  \begin{examzhsyntax}[emph={[1]column-sep}]
    column-sep = (*\meta{dimension}*)
  \end{examzhsyntax}
  选项列之间的最小间隔。默认为 |1em|。
\end{function}

\begin{function}[added = 2026-07-11]{choices/left-indent,choices/right-indent,choices/margin}
  \begin{examzhsyntax}[emph={[1]left-indent,right-indent,margin}]
    left-indent  = (*\meta{dimension}*)
    right-indent = (*\meta{dimension}*)
    margin       = (*\meta{dimension}*)
  \end{examzhsyntax}
  设置整个 \env{choices} 环境相对于当前 \tn{linewidth} 的左右缩进，默认均为 |0pt|。\opt{margin} 同时设置左右缩进；单独设置 \opt{left-indent} 或 \opt{right-indent} 时可以形成不对称版式。缩进会先从可用宽度中扣除，再参与自动列数、每列宽度和图片选项的计算，因此可以安全地与 \env{problem} 或 \env{enumerate} 的现有缩进叠加。
\end{function}

\begin{latexexample}{\opt{choices/left-indent} 和 \opt{choices/right-indent} 的效果}
  \begin{enumerate}
    \item 题干
      \begin{choices}[left-indent = 1em, right-indent = 1em]
        \item 选项一
        \item 选项二
        \item 选项三
        \item 选项四
...
\begin{function}{choices/columns}
  \begin{examzhsyntax}[emph={[1]columns}]
...
\begin{function}{choices/label-pos}
  \begin{examzhsyntax}[emph={[1]label-pos}]
    label-pos = (*<(auto)|top-left|left|bottom>*)
  \end{ccnusyntax}
  标签相对于选项内容的位置。\opt{auto} 会根据内容高度来判断，当高度到达一定程度时会判断为插图，并将 label 放置于 内容 的 \opt{bottom} 位置；\opt{top-left} 表示在内容左侧并置于顶部；\opt{left} 表示在内容左侧并置于中间；\opt{bottom} 表示在内容底部；
\end{function}


\begin{function}{choices/label-align}
  \begin{examzhsyntax}[emph={[1]label-align}]
...
\begin{function}{choices/label-sep}
  \begin{examzhsyntax}[emph={[1]label-sep}]
...
\begin{function}{choices/label-width}
  \begin{examzhsyntax}[emph={[1]label-width}]
...
\begin{function}{choices/max-columns}
  \begin{examzhsyntax}[emph={[1]max-columns}]
...
\begin{function}[added = 2022-08-28]{choices/linesep}
  \begin{examzhsyntax}[emph={[1]linesep}]
    linesep = (*\meta{dimension}*)
  \end{examzhsyntax}
  \env{choices} 环境的行数多于 1 时的行额外弹性间距。默认为 |0pt plus .5ex|。
\end{function}

\begin{function}[added = 2026-07-11]{choices/item-linesep}
  \begin{examzhsyntax}[emph={[1]item-linesep}]
    item-linesep = (*\meta{dimension}*)
  \end{examzhsyntax}
  多行选项内部相邻文本行的额外间距，默认为 |0pt|。该值同时增加到选项 coffin 内的 \tn{baselineskip} 和公式过高时使用的 \tn{lineskip}；因此不会改变单行选项或图片模式，只会参与多行文字、公式和最终选项高度的计算。它与控制不同选项行之间距离的 \opt{choices/linesep} 相互独立。
\end{function}

\begin{latexexample}{\opt{choices/item-linesep} 的效果}
  \begin{choices}[columns = 2, item-linesep = 3pt]
    \item 这是一个会自动换行的较长选项，用于展示选项内部的额外行距。
    \item 含有公式 $\displaystyle \frac{a_1+a_2}{b_1+b_2}$ 的多行选项。
  \end{choices}
\end{latexexample}
...
\begin{function}{paren}
  \begin{examzhsyntax}[emph={[1]paren}]
    paren = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-15,updated = 2022-12-25]{paren/show-paren}
  \begin{examzhsyntax}[emph={[1]show-paren}]
    show-paren = (*\TTF*)
  \end{examzhsyntax}
  是否显示 \tn{paren} 命令的括号。
\end{function}
...
\begin{function}[added = 2022-07-15]{paren/text-color}
  \begin{examzhsyntax}[emph={[1]text-color}]
    text-color = (*\meta{color}*)
  \end{examzhsyntax}
  设置 \tn{paren} 中答案的颜色。
\end{function}

\begin{function}[added = 2022-07-15,updated = 2026-07-11]{paren/type}
  \begin{examzhsyntax}[emph={[1]type}]
    type = (*<(hfill)|dotfill|none>*)
  \end{examzhsyntax}
  \tn{paren} 产生的括号是否自动排到行尾。\opt{hfill} 表示使用空白自动排到行尾；
  \opt{dotfill} 表示使用点线填充后再放置括号；\opt{none} 表示括号紧跟前面内容。
\end{function}
...
\begin{function}{fillin}
  \begin{examzhsyntax}[emph={[1]fillin}]
    fillin = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-21]{fillin/no-answer-type}
  \begin{examzhsyntax}[emph={[1]no-answer-type}]
...
\begin{function}[added = 2022-07-21]{fillin/no-answer-counter-index}
  \begin{examzhsyntax}[emph={[1]no-answer-counter-index}]
...
\begin{function}[added = 2022-07-21]{fillin/no-answer-counter-label}
  \begin{examzhsyntax}[emph={[1]no-answer-counter-label}]
...
\begin{function}[added = 2022-07-06]{fillin/width}
  \begin{examzhsyntax}[emph={[1]width}]
    width = (*<dimension>*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-30]{fillin/width-type}
  \begin{examzhsyntax}[emph={[1]width-type}]
...
\begin{latexexample}{\opt{width-type} 示例}
  ...
  测试
  \fillin[width-type = fill, width = 1.1\linewidth][答案] 测试
...
\begin{function}[added = 2022-07-15, updated = 2023-07-03]{fillin/box-color}
  \begin{examzhsyntax}[emph={[1]color}]
...
\begin{function}[added = 2022-07-30]{fillin/paren-type}
  \begin{examzhsyntax}[emph={[1]paren-type}]
    paren-type = (*<(banjiao)|quanjiao>*)
  \end{examzhsyntax}
  设置 \tn{fillin} 中 \kvopt{type}{paren} 时的括号类型。\opt{banjiao} 表示半角括号；\opt{quanjiao} 表示全角括号。
\end{function}

\begin{function}[added = 2022-07-30, updated = 2026-07-12]{fillin/depth}
  \begin{examzhsyntax}[emph={[1]depth}]
    depth = (*\meta{dimension}*)
  \end{examzhsyntax}
  设置 \kvopt{fillin/type}{line} 的基础下划线深度。显示多行答案时，模板会以该值为下限，并根据答案内容的最大深度自动增加距离；一般无需为分式手动调整。
\end{function}
...
\begin{function}[added = 2022-07-01]{solution}
  \begin{examzhsyntax}[emph={[1]solution}]
    solution = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-01,updated = 2024-02-11]{solution/show-solution}
  \begin{examzhsyntax}[emph={[1]show-solution}]
...
\begin{function}[added = 2024-02-22]{solution/pre-analysis}
  \begin{examzhsyntax}[emph={[1]pre-analysis}]
...
\begin{function}[added = 2022-07-20]{solution/parbreak}
  \begin{examzhsyntax}[emph={[1]parbreak}]
    parbreak = (*\TFF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-01]{solution/show-qed}
  \begin{examzhsyntax}[emph={[1]show-qed}]
    show-qed = (*\TTF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-02]{solution/qedsymbol}
  \begin{examzhsyntax}[emph={[1]qedsymbol}]
...
\begin{function}[added = 2022-07-01,updated = 2022-07-02]{solution/label-content}
  \begin{examzhsyntax}[emph={[1]label-content}]
...
\begin{function}[added = 2024-03-23]{solution/label-indentation}
  \begin{examzhsyntax}[emph={[1]label-indentation}]
...
\begin{function}[added = 2022-07-01,updated = 2022-07-02]{solution/label-punct}
  \begin{examzhsyntax}[emph={[1]label-punct}]
...
\begin{function}[added = 2022-07-02]{solution/score-pre-content}
  \begin{examzhsyntax}[emph={[1]score-pre-content}]
...
\begin{function}[added = 2022-07-02]{solution/score-post-content}
  \begin{examzhsyntax}[emph={[1]score-post-content}]
...
\begin{function}[added = 2022-07-02, updated = 2022-07-03]{solution/score-format}
  \begin{examzhsyntax}[emph={[1]score-format}]
...
\begin{function}[added = 2022-07-02]{solution/score-showleader}
  \begin{examzhsyntax}[emph={[1]score-showleader}]
...
\begin{function}[added = 2022-07-19]{solution/blank-type}
  \begin{examzhsyntax}[emph={[1]blank-type}]
    blank-type = (*<(none)|manual|hide>*)
  \end{ccnusyntax}
  \kvopt{solution/show-solution}{hide} 的时候是否增加一段垂直空白。比如可以留白给学生答题。
  \begin{itemize}
    \item \opt{none}: 不增加；
...
\begin{function}[added = 2022-07-19]{solution/blank-vsep}
  \begin{examzhsyntax}[emph={[1]blank-vsep}]
...
\begin{function}[added = 2022-07-04]{list}
  \begin{examzhsyntax}[emph={[1]list}]
    list = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-04]{list/step-name,list/method-name,list/case-name}
  \begin{examzhsyntax}[emph={[1]step-name,method-name,case-name}]
...
\begin{function}[added = 2022-07-04]{list/step-punct,list/method-punct,list/case-punct}
  \begin{examzhsyntax}[emph={[1]step-punct,method-punct,case-punct}]
...
\begin{function}[added = 2022-07-03, updated = 2025-11-07]{draft}
  \begin{examzhsyntax}[emph={[1]draft}]
    draft = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-03]{draft/watermark-size}
  \begin{examzhsyntax}[emph={[1]watermark-size}]
...
\begin{function}[added = 2022-07-03]{draft/show-watermark}
  \begin{examzhsyntax}[emph={[1]show-watermark}]
    show-watermark = (*\TTF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-03]{draft/show-draft}
  \begin{examzhsyntax}[emph={[1]show-draft}]
...
\begin{function}[added = 2022-07-24]{scoringbox/position}
  \begin{examzhsyntax}[emph={[1]position}]
    position = (*<(left)|right>*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-19]{select}
  \begin{examzhsyntax}[emph={[1]chinese}]
    select = (*\marg{键值列表}*)
...
\begin{function}[added = 2022-07-19]{chinese/mark-symbol}
  \begin{examzhsyntax}[emph={[1]mark-symbol}]
...
\begin{function}[added = 2022-07-21]{chinese/show-mark}
  \begin{examzhsyntax}[emph={[1]show-mark}]
    show-mark = (*\TFF*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-19, updated = 2022-07-24]{chinese/mark-position}
  \begin{examzhsyntax}[emph={[1]mark-position}]
...
\begin{function}[added = 2022-07-19]{chinese/mark-xshift}
  \begin{examzhsyntax}[emph={[1]mark-xshift}]
...
\begin{function}[added = 2022-07-19]{chinese/mark-yshift}
  \begin{examzhsyntax}[emph={[1]mark-yshift}]
...
\begin{function}[added = 2022-07-19]{chinese/separator}
  \begin{examzhsyntax}[emph={[1]separator}]
...
\begin{function}[added = 2022-07-19]{chinese/pre-content}
  \begin{examzhsyntax}[emph={[1]pre-content}]
...
\begin{function}[added = 2022-07-19]{chinese/post-content}
  \begin{examzhsyntax}[emph={[1]post-content}]
...
\begin{function}[updated = 2022-07-19]{lineto}
  \begin{examzhsyntax}[emph={[1]lineto}]
    lineto = (*\marg{键值列表}*)
...
\begin{function}{lineto/xsep}
  \begin{examzhsyntax}[emph={[1]xsep}]
    xsep = (*\meta{dimension}*)
  \end{examzhsyntax}
...
\begin{function}{lineto/ysep}
  \begin{examzhsyntax}[emph={[1]ysep}]
    ysep = (*\meta{dimension}*)
  \end{examzhsyntax}
...
\begin{function}{lineto/name}
  \begin{examzhsyntax}[emph={[1]name}]
    name = (*\meta{dimension}*)
  \end{examzhsyntax}
...
\begin{function}[updated = 2022-07-24]{material}
  \begin{examzhsyntax}[emph={[1]material}]
    material = (*\marg{键值列表}*)
...
\begin{function}[updated = 2022-07-24]{material/author}
  \begin{examzhsyntax}[emph={[1]author}]
    author = (*\meta{作者}*)
  \end{examzhsyntax}
...
\begin{function}[updated = 2022-07-24]{material/author-format}
  \begin{examzhsyntax}[emph={[1]author-format}]
...
\begin{function}[updated = 2022-07-24]{material/title-author-sep}
  \begin{examzhsyntax}[emph={[1]title-author-sep}]
...
\begin{function}[updated = 2022-07-24]{material/format}
  \begin{examzhsyntax}[emph={[1]format}]
    format = (*\meta{格式}*)
  \end{examzhsyntax}
...
\begin{function}[updated = 2022-07-24]{material/source}
  \begin{examzhsyntax}[emph={[1]source}]
    source = (*\meta{来源出处}*)
  \end{examzhsyntax}
...
\begin{function}[updated = 2022-07-24]{material/source-format}
  \begin{examzhsyntax}[emph={[1]source-format}]
...
\begin{function}[added = 2022-07-24]{material/title-material-sep}
  \begin{examzhsyntax}[emph={[1]title-material-sep}]
...
\begin{function}[added = 2022-07-26]{poem/align}
  \begin{examzhsyntax}[emph={[1]align}]
    align = (*<(l)|c|r>*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-07-24]{poem/title-poem-sep}
  \begin{examzhsyntax}[emph={[1]title-poem-sep}]
...
\begin{function}[added = 2022-07-26]{poem/zhu-circlednumber-base}
  \begin{examzhsyntax}[emph={[1]zhu-circlednumber-base}]
...
\begin{function}[updated = 2022-08-28]{multifigures}
  \begin{examzhsyntax}[emph={[1]multifigures}]
...
\begin{function}[added = 2022-08-28]{multifigures/fig-pos}
  \begin{examzhsyntax}[emph={[1]fig-pos}]
...
\begin{function}[added = 2026-07-11]{multifigures/column-gap}
  \begin{examzhsyntax}[emph={[1]column-gap}]
    column-gap = (*\meta{dimension}*)
  \end{examzhsyntax}
  设置同一行图片之间的列间距。该键只调整网格间距，不会像
  \kvopt{multifigures/hshift} 那样逐项平移图片；默认为 |6pt|。
\end{function}

多行图片需要进一步控制列宽或整体对齐时，可以使用环境的第二个可选参数，
该参数会直接透传给底层的 \pkg{tabularray} \env{tblr}。例如，下面的写法
在增大列间距的同时保持三列等宽并居中：

\begin{verbatim}
\begin{multifigures}[columns=3,column-gap=12pt]
  ...
\end{multifigures}
\end{verbatim}
...
如果需要显式指定列规格，可写成
\begin{verbatim}
\begin{multifigures}[columns=3][colspec={*{3}{X[4.5cm,c]}}]
  ...
\end{multifigures}
\end{verbatim}
其中第二个参数属于 \pkg{tabularray} 的高级接口，\opt{c} 用于保持各列内容居中；
图片数量不足一整行时仍由
\env{multifigures} 自动补齐空单元格。
...
\begin{function}[added = 2022-08-28]{multifigures/xshift,multifigures/hshift,multifigures/yshift,multifigures/vshift}
  \begin{examzhsyntax}[emph={[1]xshift,hshift,yshift,vshift}]
    xshift = (*\meta{dimension}*)
...
\begin{function}[updated = 2022-08-28]{textfigure}
  \begin{examzhsyntax}[emph={[1]textfigure}]
...
\begin{function}[added = 2022-08-28]{textfigure/text-width}
  \begin{examzhsyntax}[emph={[1]text-width}]
    text-width = (*\meta{dimension}*)
  \end{examzhsyntax}
  文本部分的 \env{varwidth} 宽度。默认为 |\textwidth|。默认情况下该显式宽度保持原有优先级；只有启用 \opt{auto-text-width} 时，左右布局才改按图片实宽自动计算。
\end{function}

\begin{function}[added = 2026-07-12]{textfigure/auto-text-width,textfigure/min-text-width,textfigure/column-gap}
  \begin{examzhsyntax}[emph={[1]auto-text-width,min-text-width,column-gap}]
    auto-text-width = (*\TTF*)
    min-text-width = (*\meta{dimension}*)
    column-gap = (*\meta{dimension}*)
  \end{examzhsyntax}
  \opt{auto-text-width} 默认为 |false|。启用后，\opt{left}、\opt{right} 及其顶部、居中、底部变体会先测量图片的实际排版宽度，再令文字宽度等于行宽减去图片宽度和 \opt{column-gap}；默认列间距为 |1em|。若剩余空间小于 \opt{min-text-width}（默认 |.35\linewidth|），则稳定回退到该最小宽度，避免正文被宽图压成过窄的一列。上下布局不参与自动计算，仍使用 \opt{text-width}。
\end{function}

\begin{function}[added = 2022-08-28]{textfigure/figure-width}
  \begin{examzhsyntax}[emph={[1]figure-width}]
...
\begin{function}[added = 2022-08-28]{textfigure/ratio,textfigure/text-ratio}
  \begin{examzhsyntax}[emph={[1]ratio,text-ratio}]
    ratio = (*\meta{比例}*)
    text-ratio = (*\meta{比例}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2022-09-18]{textfigure/parindent}
  \begin{examzhsyntax}[emph={[1]parindent}]
...
\begin{function}[updated = 2024-02-11]{calculations}
  \begin{examzhsyntax}[emph={[1]calculations}]
...
\begin{function}[added = 2024-02-11, updated = 2025-11-07]{calculations/hsep,calculations/vsep}
  \begin{examzhsyntax}[emph={[1]hsep,vsep}]
    hsep = (*\meta{dimension}*)
    vsep = (*\meta{dimension}*)
  \end{examzhsyntax}
...
\begin{function}[added = 2024-02-11, updated = 2026-07-11]{calculations/align}
  \begin{examzhsyntax}[emph={[1]align}]
    align = (*<(t)|m|b>*)
  \end{examzhsyntax}
  题干和标签的整体对齐方式。\opt{t} 表示首行基线对齐、\opt{m} 表示居中对齐、
  \opt{b} 表示末行基线对齐。
  该设置同时用于同一行各项目的垂直对齐，因此项目中公式高度不同时无需再用
  \tn{vphantom} 手工补齐；默认使用顶部对齐。
\end{function}
